...
- [ ] Write `VENDORED.md` with the table header and the convention explained at the top
- [ ] Add a `pnpm vendor:check` script (`scripts/vendor-check.ts`) that walks
...
- [ ] Vendor `src/styles/main.css` as `@status modified`, banner + `@changes` list
- [ ] Confirm the daisyUI theme list matches what the navbar theme toggle expects
      (`light --default, dark --prefersdark`); drop `cupcake`/`night` unless wanted
- [ ] Delete the now-dead inline `.details-arrow` styling if the feed-config
...
- [ ] Restructure `src/index.html` to the two-column `app-container` grid
- [ ] Move the Load dropdown into `navbar-end`; delete the `#feed-config` overlay card
- [ ] Delete the `#stop-sheet` block entirely (replaced by `#right-panel`)
- [ ] Add `#map-controls` with `#map-search` only
- [ ] Add `#right-panel` with `#sheet-top-handle`, `#panel-resizer`, `#panel-content`
- [ ] Keep `#alerts-modal` as-is; keep the file-input element if manual load needs it
...
- [ ] Vendor `src/modules/notification-system.ts` — verbatim
- [ ] Vendor `src/modules/feed-progress-indicator.ts` — verbatim
- [ ] Vendor `src/modules/panel-resizer.ts` — modified (localStorage persistence)
- [ ] Vendor `src/modules/bottom-sheet.ts` — modified (dock and TabManager removed)
- [ ] Add `resizeNow()` and `forceMapResize()` to `src/map-controller.ts`
- [ ] Wire all four in `src/index.ts`; sheet only activates under 768px
- [ ] Record all five entries in `VENDORED.md`
...
- [ ] Vendor `src/modules/theme-controller.ts` if it is richer than the 5 lines currently
      inline in `index.ts`; otherwise keep the inline version and note the deliberate skip
- [ ] Ensure `#app-version` is still populated from `__APP_VERSION__`
- [ ] Verify the shell renders with no feed loaded: map fills the left column, right
