...
| `src/modules/search-controller.ts` | `src/modules/search-controller.ts` | ba983ca | verbatim | The map search box. Data-source agnostic: entries come from `search-entries.ts`, selection hands a `PageState` back to `AppState.setFocus`. Needs `#map-search` inside `#map-search-card` |
