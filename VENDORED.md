...
| `src/modules/app-state.ts` | — | — | — | origin | Not vendored: feed selection and the boot sequence, on top of `interlocking`'s `ui/focus-controller.ts`, which owns `setFocus`, the `onFocusChange` / `onStateChange` split and the hrefs. yard-master vendors it `modified`, since a feed there is an API row rather than a `FeedSelection` of URLs
...
| `src/modules/panel-renderer.ts` | — | — | — | origin | Not vendored: the page dispatcher, the realtime index and the route-strip hover, on top of `interlocking`'s `ui/panel-host.ts`, which owns the `data-nav` dispatch, the breadcrumb header, the scroll and `<details>` restore and the ticker. yard-master vendors it `modified`
