...
| `src/modules/basemap-control.ts` | `coloring-book` | `src/modules/basemap-control.ts` | dca23b3 | modified | Appearance is injected and persisted; projection/sky block deduped; one-shot stylesheet; the route-geometry shape toggle was dropped with upstream's `f9c1f5b`. Current through `dca23b3`: every other commit on the path was already applied here, including `4a46207`'s pointer-events fix and `9ed073c`'s globe/graticule icons. `a57ada5` (always use the globe projection) is deliberately not taken, since this app persists the projection and still offers the toggle |
...
| `src/modules/about-links.ts` | `coloring-book` | `src/modules/about-links.ts` | 1c16f14 | verbatim | The About modal's shared blocks: version/source, the project and sibling-app links, the GTFS resources list, and the feedback list. Each app passes its own `AboutApp` (name, blurb, repo, sibling) and keeps its own middle sections; coloring-book adds Keyboard Shortcuts and a Map Key between Project and Resources, test-track has none |
...
| `src/modules/nav-icons.ts` | `coloring-book` | `src/modules/nav-icons.ts` | bf73e11 | verbatim | Heroicons v2 24-outline path data for the navbar and the mobile dock, plus the filled sun/moon pair. `index.ts` fills every `[data-nav-icon]` slot and the `[data-theme-swap]` label from it at boot, so the two surfaces cannot drift apart. Only the actions both apps share are drawn from here; this app's Alerts and Browse buttons have no entry upstream and keep their own markup |
