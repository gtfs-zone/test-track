/* @vendored-from coloring-book:src/modules/modal-utils.ts
   ...
   @status modified
   @changes
   - `renderCloseIcon`, `renderTriangleIcon` and `renderWarningIcon` added.
   - The modal box is `max-w-4xl w-11/12`, and the escape button draws
     `renderCloseIcon()` rather than a bare glyph.
   Every one of these is in coloring-book at HEAD now (`renderWarningIcon`
   landed there in `039b6bd`), so Phase 7's re-vendor is a straight
   fast-forward to `verbatim`. */
